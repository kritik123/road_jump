...
const turnControl = 0.2; // player turning rate
const jumpAccel = 25; // z speed added for jump
const springConstant = 0.01; // spring players pitch
const collisionSlow = 0.1; // slow down from collisions
const pitchLerp = 0.1; // rate camera pitch changes
const pitchSpringDamp = 0.9; // dampen the pitch spring
const elasticity = 1.2; // bounce elasticity
const centrifugal = 0.002; // how much turns pull player
const forwardDamp = 0.999; // dampen player z speed
const lateralDamp = 0.7; // dampen player x speed
const offRoadDamp = 0.98; // more damping when off road
const gravity = -1; // gravity to apply in y axis
const cameraTurnScale = 2; // how much to rotate camera
const worldRotateScale = 0.00005; // how much to rotate world
...
mouseDown = mousePressed = mouseUpFrames = mouseX = 0;
...
DrawText = (text, posX) => {
    ...
    context.fillStyle = LSHA(99, 0, 0, 0.5); // set font color
...
roadGenLengthMax = roadGenLength = roadGenTaper = roadGenFreqX = roadGenFreqY = roadGenScaleX = roadGenScaleY = 0; // end of section // distance left // length of taper // X wave frequency // Y wave frequency // X wave amplitude // Y wave amplitude
...
for (
    ...
    if (roadGenLength++ > roadGenLengthMax) {
        ...
        roadGenWidth = roadWidth * R(1 - d * 0.7, 3 - 2 * d); // road width
        roadGenFreqX = R(Lerp(d, 0.01, 0.02)); // X curves
        roadGenFreqY = R(Lerp(d, 0.01, 0.03)); // Y bumps
        roadGenScaleX = i > roadEnd ? 0 : R(Lerp(d, 0.2, 0.6)); // X scale
    ...
    road[i].a = road[i - 1] ?
        Math.atan2(road[i - 1].y - road[i].y, segmentLength) :
        0;
}
...
Update = () => {
    ...
    if (position.y < roadY) {
        ...
        velocity = new Vec3(0, Math.cos(roadA), Math.sin(roadA))
            .Multiply(-elasticity * dp)
            .Add(velocity);

        // apply player brake and accel
        velocity.z += mouseDown ?
            playerBrake :
    ...
    for (i = 2; i--;) {
        ...
        g = context.createRadialGradient(
            (x =
                c.width *
                (0.5 +
                    Lerp((heading / PI / 2 + 0.5 + i / 2) % 1, 4, -4) -
                    backgroundOffset)),
            (y = horizon - c.width / 5),
            c.width / 25,
            x,
    ...
    for (x = w = i = 0; i < drawDistance + 1;) {
        p = new Vec3(
                (x += w += road[s + i].x), // sum local road offsets
                road[s + i].y,
                (s + i) * segmentLength
            ) // road y and z pos
        ...
        road[s + i++].p = p // projected road point
            .Multiply(new Vec3(z, z, 1)) // projection
    ...
    for (
        i = drawDistance; i--; // iterate in reverse
            ...
            if (i % (Lerp(i / drawDistance, 1, 9) | 0) == 0) {
                ...
                if (segment1.w > 400)
                    DrawPoly(
                        p1.x,
                        p1.y,
                        p1.z * (segment1.w + curbWidth),
                        p2.x,
                        p2.y,
                        p2.z * (segment2.w + curbWidth),
                ...
                DrawPoly(
                    ...
                    p2.z * segment2.w,
                    LSHA(
                        (((s + i) * segmentLength) % checkPointDistance < 300 ? 70 : 7) +
                        light
                    )
                );
